<?php

class Pengguna extends CI_Controller
{
    protected $pagePath;
    protected $breadcrum;

    public function __construct()
    {
        parent::__construct();
        // PermissionLogin($this->session);
        $this->pagePath = 'Master/Pengguna/';
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('<text>', '</text>');
        $this->breadcrum = array(
          array('Master', '#'),
          array('Pengguna', $this->pagePath),
        );

        // CallModel
        $this->load->model('Master/PenggunaModel', 'model');
    }

    public function index()
    {
        PermissionLogin($this->session);
        $data = array();
        $data['title'] = 'Pengguna';
        $data['template'] = $this->pagePath.'index';
        $data['loader'] = array(
          'path' => base_url().$this->pagePath,
          'stylesheet' => $this->pagePath.'Loader/index_stylesheet',
          'javascript' => $this->pagePath.'Loader/index_javascript',
        );
        $data['breadcrum'] = array_merge($this->breadcrum, array(
          array('Index', ''),
        ));

        // GetModel
        $data['listIndex'] = $this->model->getAllEntries();

        $data = array_merge($data, pathHelper());
        $this->parser->parse('template', $data);
    }

    public function create()
    {
        PermissionLogin($this->session);
        $data = array(
          'id' => '',
          'idpegawai' => '',
          'username' => '',
          'password' => '',
          'idhakakses' => '',
        );

        $data['error'] = '';
        $data['title'] = 'Tambah Data';
        $data['template'] = $this->pagePath.'manage';
        $data['loader'] = array(
          'path' => base_url().$this->pagePath,
          'stylesheet' => $this->pagePath.'Loader/manage_stylesheet',
          'javascript' => $this->pagePath.'Loader/manage_javascript',
        );
        $data['breadcrum'] = array_merge($this->breadcrum, array(
          array('Tambah', ''),
        ));

        $data['listMarketing'] = $this->model->getMarketing();
        $data['listHakAkses'] = $this->model->getHakAkses();

        $data = array_merge($data, pathHelper());
        $this->parser->parse('template', $data);
    }

    public function edit($id)
    {
        PermissionLogin($this->session);
        if ($id != '') {
            $row = $this->model->getSpecifiedEntries($id);
            if (isset($row->id)) {
                $data = array(
                  'id' => $row->id,
                  'idpegawai' => $row->idpegawai,
                  'username' => $row->username,
                  'password' => '',
                  'idhakakses' => $row->idhakakses,
                );

                $data['error'] = '';
                $data['title'] = 'Ubah Data';
                $data['template'] = $this->pagePath.'manage';
                $data['loader'] = array(
                  'path' => base_url().$this->pagePath,
                  'stylesheet' => $this->pagePath.'Loader/manage_stylesheet',
                  'javascript' => $this->pagePath.'Loader/manage_javascript',
                );
                $data['breadcrum'] = array_merge($this->breadcrum, array(
                  array('Ubah', ''),
                ));

                $data['listMarketing'] = $this->model->getMarketing();
                $data['listHakAkses'] = $this->model->getHakAkses();

                $data = array_merge($data, pathHelper());
                $this->parser->parse('template', $data);
            } else {
                $this->session->set_flashdata('error', true);
                $this->session->set_flashdata('message_flash', 'data tidak ditemukan.');
                redirect($this->pagePath, 'location');
            }
        } else {
            $this->session->set_flashdata('error', true);
            $this->session->set_flashdata('message_flash', 'data tidak ditemukan.');
            redirect($this->pagePath);
        }
    }

    public function view($id)
    {
        PermissionLogin($this->session);
        if ($id != '') {
            $row = $this->model->getSpecifiedEntries($id);
            if (isset($row->id)) {
                $data = array(
                  'id' => $row->id,
                  'idpegawai' => $row->idpegawai,
                  'username' => $row->username,
                  'password' => '',
                  'idhakakses' => $row->idhakakses,
                );

                $data['error'] = '';
                $data['title'] = 'Rincian Data';
                $data['template'] = $this->pagePath.'view';
                $data['loader'] = array(
                  'path' => base_url().$this->pagePath,
                  'stylesheet' => $this->pagePath.'Loader/manage_stylesheet',
                  'javascript' => $this->pagePath.'Loader/manage_javascript',
                );
                $data['breadcrum'] = array_merge($this->breadcrum, array(
                  array('Rincian', ''),
                ));

                $data['listMarketing'] = $this->model->getMarketing();
                $data['listHakAkses'] = $this->model->getHakAkses();

                $data = array_merge($data, pathHelper());
                $this->parser->parse('template', $data);
            } else {
                $this->session->set_flashdata('error', true);
                $this->session->set_flashdata('message_flash', 'data tidak ditemukan.');
                redirect($this->pagePath, 'location');
            }
        } else {
            $this->session->set_flashdata('error', true);
            $this->session->set_flashdata('message_flash', 'data tidak ditemukan.');
            redirect($this->pagePath);
        }
    }

    public function save()
    {
        PermissionLogin($this->session);
        $this->form_validation->set_rules('username', 'Username', 'trim|required|min_length[1]');
        $this->form_validation->set_rules('password', 'Kata Sandi', 'trim|required|min_length[1]');
        $this->form_validation->set_rules('confirm_password', 'Konfirmasi Kata Sandi', 'trim|required|matches[password]');
        $this->form_validation->set_rules('idhakakses', 'Hak Akses', 'trim|required|min_length[1]');

        if ($this->form_validation->run() == true) {
            if ($this->input->post('id') == '') {
                if ($this->model->insert()) {
                    $this->session->set_flashdata('confirm', true);
                    $this->session->set_flashdata('message_flash', 'data telah disimpan.');
                    redirect($this->pagePath, 'location');
                }
            } else {
                if ($this->model->update()) {
                    $this->session->set_flashdata('confirm', true);
                    $this->session->set_flashdata('message_flash', 'data telah disimpan.');
                    redirect($this->pagePath, 'location');
                }
            }
        } else {
            $this->failedSave($this->input->post('id'));
        }
    }

    public function failedSave($id)
    {
        PermissionLogin($this->session);
        $data = $this->input->post();
        $data['error'] = validation_errors();
        $data['template'] = $this->pagePath.'manage';
        $data['loader'] = array(
          'path' => base_url().$this->pagePath,
          'stylesheet' => $this->pagePath.'Loader/manage_stylesheet',
          'javascript' => $this->pagePath.'Loader/manage_javascript',
        );

        $data['listMarketing'] = $this->model->getMarketing();
        $data['listHakAkses'] = $this->model->getHakAkses();

        if ($id == '') {
            $data['title'] = 'Tambah Data';
            $data['breadcrum'] = array_merge($this->breadcrum, array(
              array('Tambah', ''),
            ));
        } else {
            $data['title'] = 'Ubah Data';
            $data['breadcrum'] = array_merge($this->breadcrum, array(
              array('Ubah', ''),
            ));
        }

        $data = array_merge($data, pathHelper());
        $this->parser->parse('template', $data);
    }

    public function delete($id)
    {
        PermissionLogin($this->session);
        $this->model->delete($id);
        $this->session->set_flashdata('confirm', true);
        $this->session->set_flashdata('message_flash', 'data telah terhapus.');
        redirect($this->pagePath, 'location');
    }

    // Profile
    public function profile()
    {
        PermissionLogin($this->session);
        if ($this->session->userdata('userid') != '') {
            $row = $this->model->getSpecifiedEntries($this->session->userdata('userid'));
            if (isset($row->id)) {
                $data = array(
                  'id' => $row->id,
                  'idpegawai' => $row->idpegawai,
                  'username' => $row->username,
                  'password' => '',
                  'idhakakses' => $row->idhakakses,
                );

                $data['error'] = '';
                $data['title'] = 'Ubah Data';
                $data['template'] = $this->pagePath.'view';
                $data['loader'] = array(
                  'path' => base_url().$this->pagePath,
                  'stylesheet' => $this->pagePath.'Loader/manage_stylesheet',
                  'javascript' => $this->pagePath.'Loader/manage_javascript',
                );
                $data['breadcrum'] = array_merge($this->breadcrum, array(
                  array('Ubah', ''),
                ));

                $data['listMarketing'] = $this->model->getMarketing();
                $data['listHakAkses'] = $this->model->getHakAkses();

                $data = array_merge($data, pathHelper());
                $this->parser->parse('template', $data);
            } else {
                $this->session->set_flashdata('error', true);
                $this->session->set_flashdata('message_flash', 'data tidak ditemukan.');
                redirect($this->pagePath, 'location');
            }
        } else {
            $this->session->set_flashdata('error', true);
            $this->session->set_flashdata('message_flash', 'data tidak ditemukan.');
            redirect($this->pagePath, 'location');
        }
    }

    public function profileUpdate()
    {
        PermissionLogin($this->session);
        $this->form_validation->set_rules('username', 'Username', 'trim|required|min_length[1]');
        $this->form_validation->set_rules('password', 'Kata Sandi', 'trim|required|min_length[1]');
        $this->form_validation->set_rules('confirm_password', 'Konfirmasi Kata Sandi', 'trim|required|matches[password]');
        $this->form_validation->set_rules('idhakakses', 'Hak Akses', 'trim|required|min_length[1]');

        if ($this->form_validation->run() == true) {
            if ($this->model->update()) {
                $this->session->set_flashdata('confirm', true);
                $this->session->set_flashdata('message_flash', 'the data has been updated.');
                redirect($this->pagePath.'profile/'.$this->input->post('id'), 'location');
            }
        } else {
            $this->session->set_flashdata('error', true);
            $this->session->set_flashdata('message_flash', 'the data not update.');
            redirect($this->pagePath.'profile/'.$this->input->post('id'), 'location');
        }
    }

    public function login()
    {
        PermissionLoggedIn($this->session);
        $data = array(
          'username' => '',
          'remember' => false,
          'error' => ''
        );

        $data = array_merge($data, pathHelper());
        $this->load->view('Master/Pengguna/login', $data);
    }

    public function doLogin()
    {
        PermissionLoggedIn($this->session);
        if ($this->model->login()) {
            redirect('dashboard', 'location');
        } else {
            $data = array(
              'username' => $this->input->post('username'),
              'error' => 'Username atau Password salah!'
            );

            $data = array_merge($data, pathHelper());
            $this->load->view('Master/Pengguna/login', $data);
        }
    }

    public function logout()
    {
        // PermissionLoggedIn($this->session);
        $this->model->logout();
        redirect($this->pagePath.'login', 'location');
    }
}

/* End of file welcome.php */
/* Location: ./system/application/controllers/welcome.php */;
